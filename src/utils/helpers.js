...
export const generatePdf = (docType, data) => {
    ...
    doc.text(`Date of Issue: ${issueDate}`, 15, 46);
    if(dueDate) doc.text(`Due Date: ${dueDate}`, 15, 52);

    doc.text("Bill To:", 15, 65);
    doc.text(customer?.name || "N/A", 15, 71);
    doc.text(customer?.address || "N/A", 15, 77);
    doc.text(customer?.email || "N/A", 15, 83);
    ...
    doc.text(formatCurrency(totals.subTotal), 200, finalY + 10, { align: 'right' });
    doc.text(`VAT @ ${totals.taxRate}%:`, 140, finalY + 17, { align: 'right' });
    doc.text(formatCurrency(totals.taxAmount), 200, finalY + 17, { align: 'right' });

    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    doc.text(`Total:`, 140, finalY + 25, { align: 'right' });
    doc.text(formatCurrency(totals.total), 200, finalY + 25, { align: 'right' });
