...
const CustomerListView = ({ customers, setModal }) => (
    <div>
                ...
                {customers.length > 0 ? (
                    customers.map(c => (
                        <div key={c.id} className="flex justify-between items-center p-3 border-b last:border-b-0 hover:bg-gray-50 rounded-md">
                            ...
                            <button onClick={() => setModal({ type: 'customer', data: c })} className="text-gray-400 hover:text-indigo-600">
                                <Edit size={18}/>
                            </button>
                        </div>
                    ))
        ...
        <FloatingActionButton onClick={() => setModal({ type: 'customer' })} />
    </div>
);
