...
import { Loader, Plus, Trash2, ArrowLeft, Save, FileText, Send } from 'lucide-react';
import { formatCurrency, formatDate } from '../utils/helpers';

const JobDetailViewContent = ({ job, editableJob, setEditableJob, onSave, onGenerateQuote, onGenerateInvoice, isSaving, setIsSaving }) => {
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex space-x-2">
                     <button onClick={() => onGenerateQuote(job)} disabled={job.quote} className="p-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 disabled:bg-gray-300 flex items-center"><FileText size={16} className="mr-1"/>{job.quote ? 'Quoted' : 'Quote'}</button>
                     <button onClick={() => onGenerateInvoice(job)} disabled={job.invoice} className="p-2 bg-red-500 text-white rounded-md hover:bg-red-600 disabled:bg-gray-300 flex items-center"><FileText size={16} className="mr-1"/>{job.invoice ? 'Invoiced' : 'Invoice'}</button>
...
const JobDetailView = ({ job, onBack, onSave, onGenerateQuote, onGenerateInvoice, onOpenSendModal }) => {
