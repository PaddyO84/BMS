...
const JobForm = ({ data, customers, onSave, onClose }) => {
    const [job, setJob] = useState(data || { jobTitle: '', customerId: '', status: 'New', dateRequested: '' });
    ...
    return (
        <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg">
            <h2 className="text-2xl font-bold mb-6">{data.id ? 'Edit Job' : 'New Job'}</h2>
            <div className="grid grid-cols-1 gap-4">
                ...
                <input type="date" name="dateRequested" value={job.dateRequested} onChange={handleChange} className="p-2 border rounded-md" />
            </div>
